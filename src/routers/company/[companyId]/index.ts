...
import { getCompany } from "../../../actions/company/getCompany.js";
...
router
  ...
  .get(verifyToken, async (req: any, res) => {
    try {
      ...
      const { status, message, company, permissions } = await getCompany({
        userId: user.id,
        companyId,
      });

      res.status(status).send({ message, company, permissions });
    } catch (caught: any) {
      const {
